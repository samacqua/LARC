	...
	HighlightWithinTextarea.prototype = {
		...
		renderMarks: function (boundaries) {
			...
			input = input.replace(/\{\{hwt-mark-start\|(\d+)\}\}/g, function (match, submatch) {
				...
				if (className) {
					return '<a href="#" data-toggle="tooltip" title="Hooray!"><mark class="' + className + '">';
				} else {
					return '<a href="#" data-toggle="tooltip" title="Hooray!"><mark>';
				}
			});

			// replace stop tokens with closing </mark> tags
			input = input.replace(/\{\{hwt-mark-stop\}\}/g, '</mark></a>');
